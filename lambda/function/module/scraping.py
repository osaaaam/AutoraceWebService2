...
class Scraping:
    ...
    def get_header(self, soup):
        ...
        return [race_name] + wk_td_list_header

    # 同ハンデ内での内順を取得
    def get_position(self, list_hande):
        list_position = []
        for i in range(len(list_hande)):
            # 1号車の場合
            if i == 0:
                list_position.append(1)
            # 2号車以降の場合
            else:
                if list_hande[i] == list_hande[i-1]:
                    list_position.append(list_position[i-1]+1)
                else:
                    list_position.append(1)
    ...
    def get_raceinfo_url_program(self):
        ...
        for i in range(len(self.list_soup)):
            try:
                ...
                # ヘッダー
                list_header = self.get_header(soup)
                # 選手名
                ...
                # ポジション
                list_position = self.get_position(list_hande)
                ...
                self.out_list_position.append(list_position)
